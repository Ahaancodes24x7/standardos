...
class Settings(BaseSettings):
    ...
    # v3.1 (default), v3.1-gated / v3.1-hybrid (learned classifier), v3.1-dag, v3, legacy-2.1 …
    standardos_pipeline_config: str = "v3.1"

    # Serverless hosts (Vercel sets VERCEL=1) freeze a function once its response is sent, so
    # work queued after the response never finishes. There, analyses run only through
    # POST /api/runs/{id}/execute (the web client calls it) and connections are not pooled.
    ...
    def is_serverless(self) -> bool:
        ...
        return self.serverless if self.serverless is not None else bool(os.environ.get("VERCEL"))
