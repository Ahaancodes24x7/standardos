...
MAX_ATTEMPTS = 3
# The heartbeat moves at every pipeline stage and a serverless function cannot outlive its
# maximum duration (60 s on Vercel), so a run silent for 90 s has lost its worker.
    ...
    *,
    ...
    audit(db, actor_id=owner_id, document_id=doc.id, entity_type="document", entity_id=str(doc.id),
          action="uploaded", after={"name": name, "sha256": sha256, "byteSize": len(data),
                                    "documentType": document_type, "documentTypeLabel": document_type_label})
...
def _claim_run(db: Session, run_id: uuid.UUID) -> bool:
    ...
    A running run whose heartbeat is stale (its worker was frozen or killed — normal on
    serverless hosts) is reclaimed while it has attempts left, so a client retrying
    POST /runs/{id}/execute recovers it without waiting for the sweeper.
    """
    claimable = (m.AnalysisRun.status == "queued") | (
        (m.AnalysisRun.status == "running")
        & (m.AnalysisRun.attempts < MAX_ATTEMPTS)
        & ((m.AnalysisRun.heartbeat_at < now() - STALE_AFTER) | m.AnalysisRun.heartbeat_at.is_(None))
    )
    claimed = db.execute(
        update(m.AnalysisRun)
        .where(m.AnalysisRun.id == run_id, claimable)
        .values(
            status="running",
...
def execute_run(run_id: uuid.UUID, owner_id: Optional[uuid.UUID]) -> str:
    ...
    with session_scope() as db:
        ...
        try:
            ...
            config = doctypes.config_for(pipeline_config(), doc.document_type)
            result = run_pipeline(source, loaded.corpus, on_stage=on_stage, config=config)
        ...
        snapshots.append(
            {
                ...
                "documentTypeLabel": doctypes.display_label(doc.document_type, doc.document_type_label),
...
def update_document(
    ...
    *,
    ...
    """Rename or re-type a stored document. Returns a new run id when the change needs re-analysis
    (the document type changes how the pipeline reads the document)."""
    doc = db.scalar(select(m.Document).where(m.Document.id == document_id, m.Document.owner_id == owner_id))
    if not doc:
        raise NotFound("Document not found.")
    before = {"name": doc.name, "documentType": doc.document_type, "documentTypeLabel": doc.document_type_label}
    rerun = False
    if name is not None:
        cleaned = name.strip()
        if not 1 <= len(cleaned) <= 200:
            raise AppError("Document names must be 1–200 characters.")
        doc.name = cleaned
    if document_type is not None or document_type_label is not None:
        key, label = doctypes.validate(document_type or doc.document_type, document_type_label)
        rerun = doctypes.config_for(pipeline_config(), key) != doctypes.config_for(pipeline_config(), doc.document_type)
        doc.document_type, doc.document_type_label = key, label
    after = {"name": doc.name, "documentType": doc.document_type, "documentTypeLabel": doc.document_type_label}
    audit(db, actor_id=owner_id, document_id=doc.id, entity_type="document", entity_id=str(doc.id),
          action="updated", before=before, after=after)
    db.commit()
    return queue_rerun(db, document_id, owner_id) if rerun else None


def delete_document(db: Session, document_id: uuid.UUID, owner_id: uuid.UUID) -> None:
    """Delete a stored document with its runs, findings and audit trail (cascades)."""
    doc = db.scalar(select(m.Document).where(m.Document.id == document_id, m.Document.owner_id == owner_id))
    if not doc:
        raise NotFound("Document not found.")
    db.delete(doc)
