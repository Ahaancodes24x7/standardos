...
# v2: a categorical value anywhere in a sentence that names its parameter.
# The v1 rules above need one word order ("severe exposure", "TN-S system");
# tenders also write "exposure … shall be taken as severe", "the earthing
# arrangement of the supply is TN-S", "insulation shall be class F".
_CUE_AND_VALUE: list[tuple[str, re.Pattern[str], re.Pattern[str], Callable[[re.Match[str]], str]]] = [
    (
        "exposure_condition",
        re.compile(r"\bexposure\b", re.I),
        re.compile(r"\b(very\s+severe|extreme|severe|moderate|mild)\b", re.I),
        lambda m: re.sub(r"\s+", " ", m.group(1).lower()),
    ),
    (
        "earthing_system",
        re.compile(r"\b(?:earthing|grounding|earth(?:ed)?|neutral|supply\s+system|system\s+earthing)\b", re.I),
        re.compile(r"(?<![\w-])(TN-C-S|TN-S|TN-C|TT|IT)(?![\w-])"),
        lambda m: m.group(1),
    ),
    (
        "insulation_class",
        re.compile(r"\binsulat\w*", re.I),
        re.compile(r"\bclass\s*[-:]?\s*([BFH])\b", re.I),
        lambda m: f"Class {m.group(1).upper()}",
    ),
    (
        "bacteriological_quality",
...
def extract_text_parameters(text: str) -> list[TextParameter]:
    ...
    if current().text_attrs_v2:
        ...
        for parameter, cue, value_re, value in _CUE_AND_VALUE:
            if parameter in found:
                continue
            cue_m = cue.search(text)
            if not cue_m:
                continue
            # The value nearest after the cue, else the first one before it.
            after = [m for m in value_re.finditer(text) if m.start() >= cue_m.start()]
            m = after[0] if after else value_re.search(text)
            if m:
                out.append(TextParameter(parameter, value(m), m.group(0)))
    return out
