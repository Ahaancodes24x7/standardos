...
def render_pdf(text: str, running_header: str, blank_pages: int = 0, wrap: int = WRAP, footer: str = "") -> bytes:
    body: list[str] = []
    ...
    total = len(chunks) + blank_pages
    tail = [footer] if footer else []
    pages = [[running_header, "", *chunk, "", *tail, f"Page {n} of {total}"] for n, chunk in enumerate(chunks, 1)]
    # Scanned annexures: pages with no text layer at all.
    pages += [[] for _ in range(blank_pages)]
...
def render_docx(text: str) -> bytes:
    ...
    def entry(name: str) -> zipfile.ZipInfo:
        info = zipfile.ZipInfo(name, date_time=(2026, 1, 1, 0, 0, 0))  # deterministic archives
    ...
    with zipfile.ZipFile(buf, "w", zipfile.ZIP_DEFLATED) as z:
        ...
        z.writestr(entry("word/document.xml"), document)
    return buf.getvalue()
