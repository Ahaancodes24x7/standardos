...
from .units import UNIT_PATTERN, lookup_unit, to_canonical, unit_pattern_v2, unit_pattern_v3
...
def _quantity_re_v3(units: str) -> re.Pattern[str]:
    """v3: units in any case (validated by lookup_unit); "415-volt"; a current-kind suffix
    ("415 V AC ± 10 %", "50 kA rms") between the value and its tolerance."""
    u = f"(?i:{units})"
    kind = r"(?:\s*(?:AC|DC|a\.c\.|d\.c\.|rms|\(rms\)|r\.m\.s\.))?"
    return re.compile(
        rf"(?P<pm>±\s*)?(?P<a>{NUM})(?:\s*(?:(?P<ua>{u})(?![A-Za-z0-9²³]))?\s*(?P<sep>to|–|—|-|and)\s*(?P<b>{NUM}))?"
        rf"\s*(?:-(?=[A-Za-z]))?(?P<u>{u})(?![A-Za-z0-9²³]){kind}"
        rf"(?:\s*(?:±|\+/-|\+-)\s*(?P<tol>\d+(?:\.\d+)?)\s*(?P<tolu>%|{u})?)?"
    )


QUANTITY_RE_V3 = _quantity_re_v3(unit_pattern_v3())
...
def extract_quantities(text: str, base: int = 0) -> list[Quantity]:
    ...
    cfg = current()
    pattern = QUANTITY_RE_V3 if cfg.units_v3 else QUANTITY_RE_V2 if cfg.units_v2 else QUANTITY_RE
    for m in pattern.finditer(text):
        ...
        if m.group("pm"):
            ...
            if tol_unit and tol_unit.dimension == "percent" and cfg.units_v3:
                tol_unit = None  # "± 10 pct", "± 10 per cent" are relative, like "± 10 %"
            if tol_unit:
...
# v2 cue phrasings for quantity → parameter assignment (common tender wording).
EXTRA_CUES_V2: dict[str, tuple[str, ...]] = {
    "altitude": ("msl", "above sea level", "a.m.s.l", "site elevation", "height above"),
    "flow_rate": ("duty point", "capacity of", "delivery of", "output of the pump"),
    "rated_current": ("continuous rating", "current rating", "rated at", "rating of"),
    "earth_resistance": ("resistance of the earth", "earthing system", "earth pit resistance", "resistance to earth"),
...
def infer_parameter(text: str, quantity: Quantity, base: int = 0) -> Optional[InferredParameter]:
    ...
    for param in PARAMETERS:
        if quantity.dimension not in param.dimensions:
            continue
        for cue in (*param.cues, *extra.get(param.key, ())):
